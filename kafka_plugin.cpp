...
void kafka_plugin_impl::_process_applied_transaction(const trasaction_info_st &t)
{
  ...
  string transaction_metadata_json =
      "{\"trx_id\":\"" + std::to_string(trx_id_str) + "\",\"block_number\":" + std::to_string(t.block_number) + ",\"block_time\":" + std::to_string(time) +
...
{
  cfg.add_options()("accept_trx_topic", bpo::value<std::string>(),
                    "The topic for accepted transaction.")("applied_trx_topic", bpo::value<std::string>(),
                                                           "The topic for appiled transaction.")("kafka-uri,k", bpo::value<std::string>(),
